...
unsigned char sbox16[256][256];
void tbox( uint32_t  state[4][2])
{
	
	const uint32_t T0[16][16]=  {
...
{0x25afb918,0x25afa246,0x25afcd94,0x25af5123,0x25af1b5e,0x25af0000,0x25af748c,0x25af9cb7,0x25af6fd2,0x25aff365,0x25afd6ca,0x25af3ef1,0x25af87e9,0x25afe83b,0x25af25af,0x25af4a7d},
...
{0x5fc9be6d,0x5fc9adb1,0x5fc9c73f,0x5fc95fc9,0x5fc913dc,0x5fc90000,0x5fc97952,0x5fc998f6,0x5fc96a8e,0x5fc9f278,0x5fc9d4e3,0x5fc93547,0x5fc98b2a,0x5fc9e1a4,0x5fc9269b,0x5fc94c15},
{0x13dcbe6d,0x13dcadb1,0x13dcc73f,0x13dc5fc9,0x13dc13dc,0x13dc0000,0x13dc7952,0x13dc98f6,0x13dc6a8e,0x13dcf278,0x13dcd4e3,0x13dc3547,0x13dc8b2a,0x13dce1a4,0x13dc269b,0x13dc4c15},
{0x0000be6d,0x0000adb1,0x0000c73f,0x00005fc9,0x000013dc,0x00000000,0x00007952,0x000098f6,0x00006a8e,0x0000f278,0x0000d4e3,0x00003547,0x00008b2a,0x0000e1a4,0x0000269b,0x00004c15},
{0x7952be6d,0x7952adb1,0x7952c73f,0x79525fc9,0x795213dc,0x79520000,0x79527952,0x795298f6,0x79526a8e,0x7952f278,0x7952d4e3,0x79523547,0x79528b2a,0x7952e1a4,0x7952269b,0x79524c15},
...
{0x4c15be6d,0x4c15adb1,0x4c15c73f,0x4c155fc9,0x4c1513dc,0x4c150000,0x4c157952,0x4c1598f6,0x4c156a8e,0x4c15f278,0x4c15d4e3,0x4c153547,0x4c158b2a,0x4c15e1a4,0x4c15269b,0x4c154c15},
	...
	int j,i;
	uint32_t tmp1[2]={0x00,0x00};
	uint32_t tmp4[4][2]={{0x00,0x00},{0x00,0x00},{0x00,0x00},{0x00,0x00}};
...
//	 printf("\n 1:%04x \n",state[0][j]);
	 ...
	 tmp1[j]=(state[0][j]&0x0000FFFF);
//	 printf("\n bit 2: %04x \n",tmp1[j]);
	 state[0][j]=(state[0][j] >> 16);
//	 printf("\n state[0][j]: %04x \n",state[0][j]);
	 for(i=0;i<4;i++)
	 {  
	    
	 	tmp4[i][j]=(((state[0][j])>> 4*(3-i))&0x0F);
//	 	printf("\n 1: %04x \n",tmp4[i][j]);
	 	tmp4[i][j]= (tmp4[i][j] << 4);
//	 	printf("\n 2: %04x \n",tmp4[i][j]);
	 	tmp4[i][j]= tmp4[i][j]^(((tmp1[j])>> 4*(3-i))&0x0F);
//	 	printf("\n result: %04x \n",tmp4[i][j]);
...
void print_state16(uint32_t state[4][2])
{
...
void AddConstants_1col(unsigned short state[4], int r)
{
...
void SubCell_1col(unsigned short state[4])
{
    int i;
    for(i = 0; i < 4; i++)
...
void ShiftRow_1col(unsigned short state[4])
{
...
//void MixColumn16(unsigned char state[4][2])
void MixColumn_1col(unsigned short state[4]) 
{
	int i, j, k, l;
	unsigned char tmp[4];
...
void MixColumn16(uint32_t state[4][2])
{
    int i, j, k, q;
    unsigned char z[4][4];
    unsigned char tmp[4];
...
/*	AddKeyBytes_1col(state, userkey, 0);
	...
	AddConstants_1col(state, 0);
	printf("after addconstants\n");
	print_state_1col(state);
*/
//	SubCell_1col(state);
//	printf("after subcell\n");
//	print_state_1col(state);
...
/*
	...
	for(i = 0; i < RN/4; i++)
    {
		for(j = 0; j < 4; j++)
		{
			AddConstantsBytes(state, i*4+j);
			SubCellBytes(state);
			ShiftRowBytes(state);
			MixColumnBytes(state);
        }
		AddKeyBytes(state, userkey, i+1);
	}
	for(i = 0; i < 4; i++)
...
void AddKey_2col(uint32_t state[4][2], unsigned short* keyBytes, int step)
{
    int i, j;
    for(i = 0; i < 4; i++)
        for(j = 0; j < 2; j++)
...
void AddConstants_2col(uint32_t state[4][2], int r)
{
...
void SubCell_2col(uint32_t state[2][2])
{
...
void ShiftRow_2col(uint32_t state[4][2])
{
...
void MixColumn_2col(uint32_t state[4][2])
{
    int i, j, k, q;
    unsigned char z[4][4];
    unsigned char tmp[4];
...
//void print_state_2col(unsigned short state[4][2])
void print_state_2col(uint32_t state[4][2])
{
    #define HI_NIBBLE(b) (((b) >> 4) & 0x0F)
    #define LO_NIBBLE(b) ((b) & 0x0F)
...
void LED_enc_tbox(unsigned short* input, const unsigned short* userkey, int ksbits)
{
	uint32_t state[4][2];
	unsigned short keyBytes[8];	
	int i;
...
void LED_enc(unsigned char* input, const unsigned char* userkey, int ksbits)
{
	...
	for(i = 0; i < RN/4; i++){
		for(j = 0; j < 4; j++)
		{
			...
			ShiftRow(state);
		printf("pre sub+mix %i\n", i);
		print_state(state);
			SubCell(state);
			MixColumn(state);
		printf("post sub+mix %i\n", i);
		print_state(state);
		}
...
/*	c[0] = k[0] = p[0] = 0x12;
	...
	c[6] = k[6] = p[6] = 0x00;
	c[7] = k[7] = p[7] = 0x00;
        c1col[0] = p1col[0] = k1col[0] = k1col[4] = 0x0123;
        c1col[1] = p1col[1] = k1col[1] = k1col[5] = 0x4567;
        c1col[2] = p1col[2] = k1col[2] = k1col[6] = 0x89AB;
        c1col[3] = p1col[3] = k1col[3] = k1col[7] = 0xCDEF;
...
*/
		LED_enc(c, k, kbits);
                printf("\n");
		//LED_enc16(c16, k16, kbits);
		LED_enc_tbox(c16, k16, kbits);
                printf("\n");
